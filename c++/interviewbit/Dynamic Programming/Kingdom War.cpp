...
int Solution::solve(vector<vector<int>> &A)
{
    ...
    int ans = INT_MIN, row = A.size(), col = A.front().size();
    for (int i = row - 1; i >= 0; i--)
    {
        for (int j = col - 1; j >= 0; j--)
        {
            // below expression: dp[i][j]=dp[i][j]+dp[i][j+1]+dp[i+1][j]-dp[i+1][j+1]
            dp[i][j] += ((i == row - 1) ? 0 : dp[i + 1][j]) + ((j == col - 1) ? 0 : dp[i][j + 1]) - ((i == row - 1 || j == col - 1) ? 0 : dp[i + 1][j + 1]);
            ans = max(ans, dp[i][j]);
        }
    }
    return ans;
